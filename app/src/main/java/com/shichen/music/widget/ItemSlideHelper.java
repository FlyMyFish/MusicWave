 ...
 */
public class ItemSlideHelper extends RecyclerView.ItemDecoration
    ...
    /**
     * ItemSlideHelper is in idle state. At this state, either there is no related motion event by
    ...
    static final String TAG = "ItemSlideHelper";
    ...
    /**
     * Set when ItemSlideHelper is assigned to a RecyclerView.
     */
    ...
    /**
     * Developer callback which controls the behavior of ItemSlideHelper.
     */
    Callback mCallback;
    ...
    private boolean touchMenuFirst = false;
    ...
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if (mSelected == null) {
                    if (DEBUG) {
                        Log.d(TAG, "intercept: (mSelected == null)");
                    }
                    final RecoverAnimation animation = findAnimation(event);
                    if (animation != null) {
                        if (DEBUG) {
                            Log.d(TAG, "intercept: (animation != null)");
                        }
                        float itemWidth = (float) animation.mViewHolder.itemView.getWidth();
                        if ((itemWidth - mInitialTouchX) < mCallback.getMenuWidth(animation.mViewHolder)) {
                            //点击了menu的区域
                            touchMenuFirst = true;
                            return false;
                        } else {
                            if (DEBUG) {
                                Log.d(TAG, "intercept: endRecoverAnimation");
                            }
                            touchMenuFirst = false;
                ...
                if (DEBUG) {
                    Log.d(TAG, "intercept: (action == MotionEvent.ACTION_CANCEL || action == MotionEvent.ACTION_UP)");
                }
            ...
            Log.d(TAG, "intercept: return " + (mSelected != null));
        ...
        public void onTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_UP:
                    if (DEBUG) {
                        Log.d(TAG, "on touch: select(null,ACTION_STATE_IDLE)");
                    }
                    select(null, ACTION_STATE_IDLE);
                    mActivePointerId = ACTIVE_POINTER_ID_NONE;
                    break;
    ...
    /**
     * Creates an ItemSlideHelper that will work with the given Callback.
     * <p>
     * You can attach ItemSlideHelper to a RecyclerView via
     * {@link #attachToRecyclerView(RecyclerView)}. Upon attaching, it will add an item decoration,
     * an onItemTouchListener and a Child attach / detach listener to the RecyclerView.
     *
     * @param callback The Callback which controls the behavior of this touch helper.
     */
    public ItemSlideHelper(Callback callback) {
        mCallback = callback;
    }
    ...
    /**
     * Attaches the ItemSlideHelper to the provided RecyclerView. If TouchHelper is already
     * attached to a RecyclerView, it will first detach from the previous one. You can call this
     * method with {@code null} to detach it from the current RecyclerView.
     *
     * @param recyclerView The RecyclerView instance to which you want to add this helper or
     *                     {@code null} if you want to remove ItemSlideHelper from the current
     *                     RecyclerView.
     */
    ...
    private void initGestureDetector() {
        ...
        mGestureDetector = new GestureDetectorCompat(mRecyclerView.getContext(),
                new ItemSlideHelperGestureListener());
    }
    ...
    void select(ViewHolder selected, int actionState) {
        if (selected == mSelected && actionState == mActionState) {
            if (DEBUG) {
                Log.d(TAG, "select: (selected == mSelected && actionState == mActionState)");
            }
            return;
        }
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                switch (swipeDir) {
                    ...
                    case END:
                        if (DEBUG) {
                            Log.d(TAG, "select: (swipeDir case END)");
                        }
                        targetTranslateY = 0;
                    ...
                    case DOWN:
                        if (DEBUG) {
                            Log.d(TAG, "select: (swipeDir case DOWN)");
                        }
                        targetTranslateX = 0;
                    ...
                    default:
                        if (DEBUG) {
                            Log.d(TAG, "select: (swipeDir default)");
                        }
                        targetTranslateX = 0;
                        targetTranslateY = 0;
                }
                ...
                if (DEBUG) {
                    Log.d(TAG, "select: (RecoverAnimation currentTranslateX = " + currentTranslateX + ")");
                }
                final RecoverAnimation rv = new RecoverAnimation(prevSelected, animationType,
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (mOverdrawChild == prevSelected.itemView) {
                            Log.d(TAG, "(mOverdrawChild == prevSelected.itemView)");
        ...
        if (rvParent != null) {
            if (DEBUG) {
                Log.d(TAG, "select: (rvParent != null)" + (mSelected != null));
            }
    ...
    void endAllRecoverAnimations() {
        final int recoverAnimSize = mRecoverAnimations.size();
        for (int i = recoverAnimSize - 1; i >= 0; i--) {
            final RecoverAnimation anim = mRecoverAnimations.get(i);
            if (anim.mViewHolder.itemView.getScrollX() != 0) {
                if (!anim.mEnded) {
                    anim.cancel();
                }
                mRecoverAnimations.remove(i);
                anim.mViewHolder.itemView.scrollTo(0, 0);
            }
        }
        touchMenuFirst=false;
    }
    ...
    boolean checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        if (mSelected != null || action != MotionEvent.ACTION_MOVE
            ...
            Log.d(TAG,"checkSelectForSwipe: (mSelected != null || action != MotionEvent.ACTION_MOVE\n" +
                    "                || mActionState == ACTION_STATE_DRAG || !mCallback.isItemViewSwipeEnabled())");
            return false;
        }
        if (mRecyclerView.getScrollState() == SCROLL_STATE_DRAGGING) {
            Log.d(TAG,"checkSelectForSwipe: (mRecyclerView.getScrollState() == SCROLL_STATE_DRAGGING)");
            return false;
        }
        final ViewHolder vh = findSwipedView(motionEvent);
        if (vh == null) {
            Log.d(TAG,"checkSelectForSwipe: (vh == null)");
            return false;
        }
        ...
        if (swipeFlags == 0) {
            Log.d(TAG,"checkSelectForSwipe: (swipeFlags == 0)");
            return false;
        }
        if (touchMenuFirst) {
            //如果首次接触到的是menu，则不能执行后续的move逻辑
            return false;
        }
    ...
    /**
     * Starts dragging the provided ViewHolder. By default, ItemSlideHelper starts a drag when a
     * View is long pressed. You can disable that behavior by overriding
     * {@link ItemSlideHelper.Callback#isLongPressDragEnabled()}.
     * <p>
     ...
     * ItemSlideHelper
     * is attached.</li>
     * <li>{@link ItemSlideHelper.Callback} must have dragging enabled.</li>
     * <li>There must be a previous touch event that was reported to the ItemSlideHelper
     ...
     *                 mItemSlideHelper.startDrag(viewHolder);
     *             }
     *             return false;
     *         }
     *     });
     ...
     *                   RecyclerView.
     * @see ItemSlideHelper.Callback#isItemViewSwipeEnabled()
     */
    public void startDrag(ViewHolder viewHolder) {
        ...
        if (viewHolder.itemView.getParent() != mRecyclerView) {
            Log.e(TAG, "Start drag has been called with a view holder which is not a child of "
                    + "the RecyclerView which is controlled by this ItemSlideHelper.");
            return;
        }
        obtainVelocityTracker();
        mDx = mDy = 0f;
    ...
    /**
     * Starts swiping the provided ViewHolder. By default, ItemSlideHelper starts swiping a View
     * when user swipes their finger (or mouse pointer) over the View. You can disable this
     * behavior
     * by overriding {@link ItemSlideHelper.Callback}
     * <p>
     * For this method to work:
     * <ul>
     * <li>The provided ViewHolder must be a child of the RecyclerView to which this
     * ItemSlideHelper is attached.</li>
     * <li>{@link ItemSlideHelper.Callback} must have swiping enabled.</li>
     * <li>There must be a previous touch event that was reported to the ItemSlideHelper
     * through RecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
     * grabs previous events, this should work as expected.</li>
     * </ul>
     ...
     *         public boolean onTouch(View v, MotionEvent event) {
     *             if (MotionEvent.getActionMasked(event) == MotionEvent.ACTION_DOWN) {
     *                 mItemSlideHelper.startSwipe(viewHolder);
     *             }
     *             return false;
     *         }
     *     });
    ...
    public void startSwipe(ViewHolder viewHolder) {
        ...
        if (viewHolder.itemView.getParent() != mRecyclerView) {
            Log.e(TAG, "Start swipe has been called with a view holder which is not a child of "
                    + "the RecyclerView controlled by this ItemSlideHelper.");
            return;
        }
        obtainVelocityTracker();
        mDx = mDy = 0f;
     ...
     * {@link ItemSlideHelper}.
     */
    public interface ViewDropHandler {

        /**
         * Called by the {@link ItemSlideHelper} after a View is dropped over another View.
         * <p>
    ...
    /**
     * This class is the contract between ItemSlideHelper and your application. It lets you control
     ...
     * {@link SimpleCallback}.
     * <p>
     * If user drags an item, ItemSlideHelper will call
     ...
     * custom drop target.
     * <p>
     * When a View is swiped, ItemSlideHelper animates it until it goes out of bounds, then calls
    ...
    public abstract static class Callback {
        ...
        private static final ItemSlideUIUtil sUICallback;
        ...
        static {
            if (Build.VERSION.SDK_INT >= 21) {
                sUICallback = new ItemSlideUIUtilImpl.Api21Impl();
            } else {
                sUICallback = new ItemSlideUIUtilImpl.BaseImpl();
            }
        }

        /**
         * Returns the {@link ItemSlideUIUtil} that is used by the {@link Callback} class for
         * visual
         * changes on Views in response to user interactions. {@link ItemSlideUIUtil} has different
         ...
         * </pre>
         *
         * @return The {@link ItemSlideUIUtil} instance that is used by the {@link Callback}
         */
        public static ItemSlideUIUtil getDefaultUIUtil() {
            return sUICallback;
        }
         ...
         * swiping.
         *
         * @param recyclerView The RecyclerView to which ItemSlideHelper is attached.
         ...
         *
         * @param recyclerView The RecyclerView to which ItemSlideHelper is attached to.
        ...
        /**
         * Called when ItemSlideHelper wants to move the dragged item from its old position to
         * the new position.
         * <p>
         * If this method returns true, ItemSlideHelper assumes {@code viewHolder} has been moved
        ...
        /**
         * Returns whether ItemSlideHelper should start a drag and drop operation if an item is
         * long pressed.
         * <p>
         * Default value returns true but you may want to disable this if you want to start
         * dragging on a custom view touch using {@link #startDrag(ViewHolder)}.
         *
         * @return True if ItemSlideHelper should start dragging an item when it is long pressed,
        ...
        /**
         * Returns whether ItemSlideHelper should start a swipe operation if a pointer is swiped
         * over the View.
         * <p>
         * Default value returns true but you may want to disable this if you want to start
         * swiping on a custom view touch using {@link #startSwipe(ViewHolder)}.
         *
         * @return True if ItemSlideHelper should start swiping an item when user swipes a pointer
        ...
        /**
         * When finding views under a dragged view, by default, ItemSlideHelper searches for views
        ...
        public float getMenuWidth(ViewHolder viewHolder) {
            return 0.0f;
        }

        /**
         * Returns the fraction that the user should move the View to be considered as it is
         * dragged. After a view is moved this amount, ItemSlideHelper starts checking for Views
         ...
         * Keep in mind that ItemSlideHelper also checks the perpendicular velocity and makes sure
        ...
        /**
         * Defines the maximum velocity ItemSlideHelper will ever calculate for pointer movements.
         * <p>
         * To consider a movement as swipe, ItemSlideHelper requires it to be larger than the
         ...
         *
         * @param defaultValue The default value(in pixels per second) used by the ItemSlideHelper.
        ...
        /**
         * Called by ItemSlideHelper to select a drop target from the list of ViewHolders that
         ...
         * <p>
         * ItemSlideHelper will keep a reference to the View until it is detached from
         * RecyclerView.
         * As soon as it is detached, ItemSlideHelper will call
        ...
        /**
         * Called when the ViewHolder swiped or dragged by the ItemSlideHelper is changed.
         * <p/>
         * If you override this method, you should call super.
         *
         * @param viewHolder  The new ViewHolder that is being swiped or dragged. Might be null if
         *                    it is cleared.
         * @param actionState One of {@link ItemSlideHelper#ACTION_STATE_IDLE},
         *                    {@link ItemSlideHelper#ACTION_STATE_SWIPE} or
         *                    {@link ItemSlideHelper#ACTION_STATE_DRAG}.
         ...
         * <p>
         * ItemSlideHelper does not create an extra Bitmap or View while dragging, instead, it
         ...
         *
         * @param recyclerView The RecyclerView controlled by the ItemSlideHelper.
        ...
        void onDraw(Canvas c, RecyclerView parent, ViewHolder selected,
                    List<ItemSlideHelper.RecoverAnimation> recoverAnimationList,
                    int actionState, float dX, float dY) {
            final int recoverAnimSize = recoverAnimationList.size();
            for (int i = 0; i < recoverAnimSize; i++) {
                final ItemSlideHelper.RecoverAnimation anim = recoverAnimationList.get(i);
                anim.update();
        ...
        /**
         * Called by the ItemSlideHelper when the user interaction with an element is over and it
         ...
         *
         * @param recyclerView The RecyclerView which is controlled by the ItemSlideHelper.
        ...
        /**
         * Called by ItemSlideHelper on RecyclerView's onDraw callback.
         * <p>
         ...
         * <code>dY</code>.
         * ItemSlideHelper also takes care of drawing the child after other children if it is being
        ...
        /**
         * Called by the ItemSlideHelper when user action finished on a ViewHolder and now the View
         ...
         *
         * @param recyclerView  The RecyclerView to which the ItemSlideHelper is attached to.
        ...
        /**
         * Called by the ItemSlideHelper when user is dragging a view out of bounds.
         * <p>
         ...
         *
         * @param recyclerView        The RecyclerView instance to which ItemSlideHelper is
         *                            attached to.
     ...
     *
     * <pre>
     * ItemSlideHelper mIth = new ItemSlideHelper(
     *     new ItemSlideHelper.SimpleCallback(ItemSlideHelper.UP | ItemSlideHelper.DOWN,
     *         ItemSlideHelper.LEFT) {
     *         public abstract boolean onMove(RecyclerView recyclerView,
     *             ViewHolder viewHolder, ViewHolder target) {
    ...
    private class ItemSlideHelperGestureListener extends GestureDetector.SimpleOnGestureListener {

        ItemSlideHelperGestureListener() {
        }
            ...
            mValueAnimator.addUpdateListener(
                    new ValueAnimator.AnimatorUpdateListener() {
                        ...
                        public void onAnimationUpdate(ValueAnimator animation) {
                            Log.d(TAG, "AnimatorUpdate: animation.getAnimatedFraction() = " + animation.getAnimatedFraction());
